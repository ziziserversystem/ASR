...
class Loader extends PluginBase{
	...
	***************************/
	...
	public function getTimer(){
		if(isset($this->time_count['time'])){
			...
			$this->setTimer($this->restart_time, "分");
			return $this->time_count['time'];
		}
	}
	...
	*/
	...
	public function start($time_target){
		...
		$this->broadcast("§f再起動まで 残り$time_target$offset");
		if($time_target == 1){
	...
	public function count_down($seconds){
		if($seconds == 1){
			foreach($this->getServer()->getOnlinePlayers() as $p){
				$p->kick("§a【運営】 §fサーバーが再起動されました");
			}
			$this->getServer()->shutdown();
			return true;
		}else{
			$seconds--;
			$this->setTimer($seconds, "秒");
			if($seconds == 30) $this->broadcast("§f再起動まで 残り{$seconds}秒");
			if($seconds == 10) $this->broadcast("§f再起動まで 残り{$seconds}秒");
			if($seconds < 6) $this->broadcast("§f再起動まで 残り{$seconds}秒");
	...
	************************/
	...
	public function loadConfigurations(){
		if(!file_exists($this->getDataFolder())){
			...
			$this->preferences->set("Prefix", "§a［運営］");
